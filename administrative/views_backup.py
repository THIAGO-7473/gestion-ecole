...
from .models import (
    ...
    Inscription, Examen, Note, Bulletin, Contrat, Conge, Formation, ParticipationFormation, RapportAdministratif, DocumentContrat, AttestationParticipation, DocumentConge, DocumentRapport
)
from .forms import (
...
def contrat_create(request):
    print("=== DÉBUT contrat_create ===")  # Debug
    if request.method == 'POST':
        ...
        # Créer le formulaire sans le champ documents pour éviter les erreurs de validation
        form_data = request.POST.copy()
        form = ContratForm(form_data, request.FILES)

        # Supprimer les erreurs du champ document_pdf pour permettre la validation
        if 'document_pdf' in form.errors:
            del form.errors['document_pdf']
        ...
        if form.is_valid():
            ...
            try:
                contrat = form.save()
                print(f"Contrat créé avec succès: ID={contrat.id}")  # Debug

                # Gérer les fichiers uploadés directement depuis request.FILES
                if request.FILES:
                    print(f"Fichiers détectés dans request.FILES: {request.FILES}")  # Debug
                    fichiers = request.FILES.getlist('document_pdf')
                    ...
                    for i, fichier in enumerate(fichiers):
                        ...
                        if fichier:  # Vérifier que le fichier n'est pas vide
                            print(f"Création DocumentContrat pour: {fichier.name}")  # Debug
                            try:
                                ...
                                print(f"DocumentContrat créé avec succès pour: {fichier.name}")  # Debug
                            except Exception as doc_error:
                                print(f"Erreur lors de la création du DocumentContrat: {str(doc_error)}")  # Debug
                                messages.error(request, f'Erreur lors de la création du document {fichier.name}: {str(doc_error)}')
                        else:
                ...
                print(f"Erreur lors de la création du contrat: {str(e)}")  # Debug
    ...
    print("=== FIN contrat_create ===")  # Debug
    return render(request, 'administrative/contrat_form.html', {'form': form})

@login_required
def contrat_update(request, pk):
    print("=== DÉBUT contrat_update ===")  # Debug
    contrat = get_object_or_404(Contrat, pk=pk)
    print(f"Modification du contrat ID: {pk}")  # Debug
    ...
    if request.method == 'POST':
        ...
        # Créer le formulaire sans le champ documents pour éviter les erreurs de validation
        form_data = request.POST.copy()
        form = ContratForm(form_data, request.FILES, instance=contrat)
        ...
        if form.is_valid():
            ...
            try:
                contrat = form.save()
                print(f"Contrat mis à jour avec succès: ID={contrat.id}")  # Debug
                ...
                print(f"Erreur lors de la mise à jour du contrat: {str(e)}")  # Debug
                print(f"Type d'erreur: {type(e)}")  # Debug
                import traceback
                print(f"Traceback: {traceback.format_exc()}")  # Debug
    ...
    print("=== FIN contrat_update ===")  # Debug
...
def participation_create(request):
    print("=== DÉBUT participation_create ===")  # Debug
    if request.method == 'POST':
        print(f"POST data: {request.POST}")  # Debug
        print(f"FILES data: {request.FILES}")  # Debug
        print(f"Nombre de fichiers: {len(request.FILES)}")  # Debug

        # Créer le formulaire sans le champ attestation_files pour éviter les erreurs de validation
        form_data = request.POST.copy()
        form = ParticipationForm(form_data, request.FILES)

        # Supprimer les erreurs du champ attestation_files pour permettre la validation
        if 'attestation_files' in form.errors:
            del form.errors['attestation_files']
        ...
        if form.is_valid():
            ...
            try:
                ...
                print(f"Participation créée avec succès: ID={participation.id}")  # Debug
                ...
                if request.FILES:
                    ...
                    for i, fichier in enumerate(fichiers):
                        print(f"Traitement du fichier {i+1}: {fichier.name}")  # Debug
                        if fichier:  # Vérifier que le fichier n'est pas vide
                            print(f"Création AttestationParticipation pour: {fichier.name}")  # Debug
                            try:
                                ...
                                print(f"AttestationParticipation créée avec succès pour: {fichier.name}")  # Debug
                            except Exception as doc_error:
                                print(f"Erreur lors de la création de l'AttestationParticipation: {str(doc_error)}")  # Debug
                                messages.error(request, f'Erreur lors de la création de l\'attestation {fichier.name}: {str(doc_error)}')
                        else:
                            print(f"Fichier ignoré (vide): {fichier}")  # Debug
                else:
                    print("Aucun fichier détecté dans request.FILES")  # Debug
                ...
                print(f"Erreur lors de la création de la participation: {str(e)}")  # Debug
                print(f"Type d'erreur: {type(e)}")  # Debug
                import traceback
                print(f"Traceback: {traceback.format_exc()}")  # Debug
                messages.error(request, f'Erreur lors de la création de la participation: {str(e)}')
        else:
            print(f"Form errors: {form.errors}")  # Debug
            print(f"Form non_field_errors: {form.non_field_errors()}")  # Debug
            for field, errors in form.errors.items():
...
def participation_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        # Créer le formulaire sans le champ attestation_files pour éviter les erreurs de validation
        form_data = request.POST.copy()
        form = ParticipationForm(form_data, request.FILES, instance=participation)
        ...
        if form.is_valid():
            ...
            try:
                participation = form.save()
                print(f"Participation mise à jour avec succès: ID={participation.id}")  # Debug
                ...
                print(f"Erreur lors de la mise à jour de la participation: {str(e)}")  # Debug
                print(f"Type d'erreur: {type(e)}")  # Debug
                import traceback
                print(f"Traceback: {traceback.format_exc()}")  # Debug
    ...
    print("=== FIN participation_update ===")  # Debug
...
def rapport_create(request):
    print("=== DÉBUT rapport_create ===")  # Debug
    if request.method == 'POST':
        print(f"POST data: {request.POST}")  # Debug
        print(f"FILES data: {request.FILES}")  # Debug
        print(f"Nombre de fichiers: {len(request.FILES)}")  # Debug
        ...
        # Créer le formulaire sans le champ documents pour éviter les erreurs de validation
        form_data = request.POST.copy()
        form = RapportAdministratifForm(form_data, request.FILES)
        ...
        if form.is_valid():
            ...
            try:
                rapport = form.save()
                print(f"Rapport créé avec succès: ID={rapport.id}")  # Debug
                ...
                if request.FILES:
                    ...
                    for i, fichier in enumerate(fichiers):
                        print(f"Traitement du fichier {i+1}: {fichier.name}")  # Debug
                        if fichier:  # Vérifier que le fichier n'est pas vide
                            print(f"Création DocumentRapport pour: {fichier.name}")  # Debug
                            try:
                                DocumentRapport.objects.create(
                                    rapport=rapport,
                                    fichier=fichier,
                                    nom=fichier.name
                                )
                                print(f"DocumentRapport créé avec succès pour: {fichier.name}")  # Debug
                            except Exception as doc_error:
                                print(f"Erreur lors de la création du DocumentRapport: {str(doc_error)}")  # Debug
                                messages.error(request, f'Erreur lors de la création du document {fichier.name}: {str(doc_error)}')
                        else:
                            print(f"Fichier ignoré (vide): {fichier}")  # Debug
                else:
                ...
                print(f"Erreur lors de la création du rapport: {str(e)}")  # Debug
                print(f"Type d'erreur: {type(e)}")  # Debug
                import traceback
                print(f"Traceback: {traceback.format_exc()}")  # Debug
...
def rapport_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        # Créer le formulaire sans le champ documents pour éviter les erreurs de validation
        form_data = request.POST.copy()
        form = RapportAdministratifForm(form_data, request.FILES, instance=rapport)
        ...
        if form.is_valid():
            ...
            try:
                rapport = form.save()
                print(f"Rapport mis à jour avec succès: ID={rapport.id}")  # Debug
                ...
                print(f"Erreur lors de la mise à jour du rapport: {str(e)}")  # Debug
                print(f"Type d'erreur: {type(e)}")  # Debug
                import traceback
                print(f"Traceback: {traceback.format_exc()}")  # Debug
    ...
    print("=== FIN rapport_update ===")  # Debug
